...
const state = {
  ...
  // Authentication is cookie-backed; bearer tokens are memory-only fallback credentials.
...
async function api(path, opts={}) {
  ...
  if(opts.method && !['GET','HEAD','OPTIONS'].includes(String(opts.method).toUpperCase())) {
    const csrf=document.cookie.split(';').map(v=>v.trim()).find(v=>v.startsWith('sdpstudio_csrf='));
    if(csrf) headers['X-CSRF-Token']=decodeURIComponent(csrf.slice('sdpstudio_csrf='.length));
  }
  if(state.authToken) headers.Authorization=`Bearer ${state.authToken}`;
  const res = await fetch(path, {...fetchOpts, headers, credentials:'same-origin'});
